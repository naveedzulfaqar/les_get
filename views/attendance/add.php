...
<html lang="en">
    ...
    <body>
            ...
            <form class="form-horizontal" role="form" action="" method="post" enctype="multipart/form-data" id="modifyattendancepopup"> 
                <div class="table-responsive">
                    <table class="table table-striped table-bordered" width="50%" cellspacing="0">       
                        ...
                        <td style="text-align:center">
                            <div class="input-group bootstrap-timepicker timepicker">
                                <input id="timepicker2" type="text" class="form-control input-small" onchange="get_attendance_details()">
                                <span class="input-group-addon"><i class="glyphicon glyphicon-time"></i></span>
                            </div>
                        </td>
                        </tr>
        ...
        <script>
            ...
            /*$(document).ready(function() {
             $("input[name='get_attendance']").on('click', function(e) {
             e.preventDefault();
             var comments = $("#comments").val(); 
             $('#comments').css('border-color', 'green');
             var error = 0;
             ...
             $('#a_loader').show();
             var url = window.opener.location.href;
             $.post("", {timein: $("#timein").val(), timeout: $("#timeout").val(), EmployeeID: $("#EmployeeID").val(), AID: $("#AID").val(), ShiftID: $("#ShiftID").val(), Day: $("#Day").val(), date: $("#date").val(), comments: $("#comments").val()}, function(data) {
             if (url.indexOf("shiftreports") == '-1') {
             window.opener.document.getElementById("bttnatt").click();
             }

...
<html lang="en">
    ...
    <body>
        ...
        <div class="container content">
            ...
            ?>
            <h3 style="font-size: 23px; color: #2fa4e7;"><span><?php echo $eod . ' EOD of ' . $attendance_details['FirstName'] . ' ' . $attendance_details['LastName']; ?></span></h3>
            ...
            <form class="form-horizontal" role="form" action="" method="post" enctype="multipart/form-data" id="modifyattendancepopup"> 
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered" width="50%" cellspacing="0">       
                            ...
                            <tbody>
                                <tr id="">
                                    <?php
                                    if ($this->session->userdata('id') == $attendance_details['EmployeeID']) {
                                        if (count($attendance_details['ChatRecord']) == 0) {
                                            $disabled = '';
                                        } else {
                                    ...
                                    ?>
                                    <td>
                                        <input type="number" name="TotalChats" class="form-control" id="totalchat" value="<?php echo $attendance_details['ChatRecord']['TotalChats']; ?>"  placeholder="Total Chats" <?php echo $disabled; ?> >
                                    </td> 
                                    <td>
                                        <input type="number" name="TotalBillableChats" class="form-control" id="totalbilablechat" value="<?php echo $attendance_details['ChatRecord']['TotalBillableChats']; ?>"  placeholder="Total Billable Chats" <?php echo $disabled; ?> >
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                ...
                <div class="form-group"> 
                    <div class="col-lg-6">
                        <label> 
                            <?php echo $eod; ?> EOD<span class="mandatory">*</span>
                        </label>
                        <textarea class="form-control" id="eod" rows="3" name="eod" ><?php echo $attendance_details['EOD']; ?></textarea><!--required-->
                    </div>
                </div>
                <input type="button" id="get_eod" name="get_eod" class="btn btn-primary btn-lg btn-block" value="<?php echo $eod; ?> EOD" style="font:menu; font-size:14px" onClick="validateForm()"></input>
        ...
        <script>
            ...
            function validateForm() {
                ...
                if (error == 0) {
                    ...
                    $.post("", {AID: $("#AID").val(), Status: $("#Status").val(), TotalChats: $("#totalchat").val(), TotalBillableChats: $("#totalbilablechat").val(), CompanyID: $("#CompanyID").val(), DepartmentID: $("#DepartmentID").val(), eod: $("#eod").val()}, function (data) {
                        if (url.indexOf("shiftreports") == '-1') {
                            window.opener.document.getElementById("bttnatt").click();
                        }
